...
main() {
    ...
    if [ -n "$PG_PASS" ]; then
        ...
        echo ""
        echo -e "📝 ${CYAN}Stunnel Client Config (TLS - Default):${NC}"
        echo -e "   [postgres-client]"
        echo -e "   client  = yes"
        echo -e "   accept  = 127.0.0.1:5432"
        echo -e "   connect = ${DOMAIN}:${STUNNEL_PORT}"
        echo -e "   CAfile  = /path/to/ca-cert.pem"
        echo -e "   verify  = 2"
        echo ""
        echo -e "📦 ${CYAN}Required CA Certificate:${NC}"
        echo -e "   ${YELLOW}deploy/docker/certs/ca-cert.pem${NC}"
        echo ""
        echo -e "🔒 ${CYAN}mTLS (Optional - if server requires):${NC}"
        echo -e "   Add to config: cert = /path/to/client-cert.pem"
        echo -e "   Add to config: key  = /path/to/client-key.pem"
    else
